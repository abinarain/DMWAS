...
#Description: This script creates logistic regression of an encoded Variant file having SNPs and/or DIPs
...
#Example: python3 createLogitReg.py NullMafMultiColDIPsScoredEncoded.txt test.fam 5 
...
lr=LogisticRegression(solver="newton-cg") #Using Newton-CG solver in the logistic Regression
lr.fit(xtrain,ytrain) #Use the object to create a fit object
...
#plt.show()
plt.savefig('ConfusionLogit.png', bbox_inches='tight')
print("The Confusion Matrix is plotted as ConfusionLogit.png")
...
#plt.show()
plt.savefig('ROCCurveLogit.png', bbox_inches='tight')
print("The ROC Curve is plotted as ROCCurveLogit.png")
plt.clf() #clear the figure
plt.cla() #clear the axes
...
for i in range(len(data.columns)):
    b=partial_dependence(lr,features=[i],X=data,percentiles=(0,1))
...
#print(my_df.head())
my_df.to_csv('PDValuesLR.csv', index=False)
print("The partial dependencies of feature variables are stored in PDValuesLR.csv")
#a.sort(axis=0)
#np.savetxt('columgSig.csv', listt, delimiter=',')  
print("Columns Significance Saved to PDValuesLR.csv")
...
plt.legend(loc = 'best')
#plt.show()
plt.savefig('PRCurveLogit.png', bbox_inches='tight')
print("The Precision Recall curve plotted as PRCurveLogit.png")
plt.close()
